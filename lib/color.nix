...
# Adapted from https://github.com/bertof/nix-rice/blob/ac810023c1bb3d999ab4295ea4eb261412f4b7c0/color.nix
  let
    ...
    _toShortHex = values:
      assert (all _is8Bit values); ''${concatMapStrings (v: fixedWidthString 2 "0" (hex.fromInt (float.round (0.0 + v)))) values}'';
    ...
    _relative_operator = operator: prev: value: let
      percentMatches = strings.match "[[:space:]]*(-?[[:digit:]]+\.?[[:digit:]]*)%[[:space:]]*" value;
      percentValue =
        if percentMatches != null
        then fromJSON (head percentMatches)
        else null;
    in
      if isNumber value
      then operator prev value # If the input is a number, apply it directly
  ...
  in rec {
    ...
    rgba' = r: g: b: a: rgba {inherit r g b a;};
    rgb' = r: g: b: rgba {inherit r g b;};
    ...
    rgbaToHsla = color: let
      ...
      hue =
        (
          ...
          then (float.mod' (((g - b) / delta) + 6) 6)
          else if g == c_max
          then (b - r) / delta + 2
          else assert b == c_max; (r - g) / delta + 4
        )
        * 60;
      ...
      saturation =
        if delta == 0.0
        then 0.0
        else delta / (1 - abs (2.0 * lightness - 1.0));
    in
      assert (isRgba color);
        hsla {
    ...
    hslaToRgba = color:
      assert (isHsla color); let
        ...
        r' =
          if inRange 120 240 h
          then 0
          else if inRange 60 300 h
          then x
          else c;
        g' =
          if inRange 60 180 h
          then c
          else if inRange 0 240 h
          then x
          else 0;
        b' =
          if inRange 180 300 h
          then c
          else if inRange 120 360 h
          then x
          else 0;
      in
        rgba {
    ...
    rgbaToCSS = color:
      assert (isRgba color); "rgba(${toString color.r}, ${toString color.g}, ${toString color.b}, ${toString color.a})";
  }
