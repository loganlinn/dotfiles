...
with lib; rec {
  ...
  filter = {
    ...
    assert _pathIsDirectory root; let
  ...
  inDirectory = directory: args: let
  ...
  in
    ...
    then f args
      ...
      path: type:
        (
          if pathIsDirectory
          then inDirectory path_ args path type
          else path_ == path
        )
        || args.matchParents
        && type == "directory"
  ...
  _toCleanPath = absPath: path:
    assert _pathIsDirectory absPath;
      ...
      then
        ...
        then path
        else toString (absPath + ("/" + path))
      else throw "unsupported type ${builtins.typeOf path}, expected string or path";
