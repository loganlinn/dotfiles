#!/usr/bin/env bash
#
# Adapted from https://github.com/i3/i3/issues/838#issuecomment-481531210
#
...
markQueue() {
	...
	for i in "${queueCopy[@]}"; do
		...
		if i3-msg "[id=$i] mark --add _prevFocus$j" 2>/dev/null | grep -i "\"success\":true" 1>/dev/null 2>&1; then
			queue=("${queue[@]:0}" "$i")
			j=$((j + 1))
		fi
	done
...
xprop -root -spy _NET_ACTIVE_WINDOW |
	while read -r line; do
		...
		if [[ $period -gt $permanence || "$currentFocus" = "${queue[0]}" ]]; then
