...
# Default comment highlight style is black, which is nearly invisible if using dark background.
typeset -gA ZSH_HIGHLIGHT_STYLES
ZSH_HIGHLIGHT_STYLES[comment]='fg=8'

# Toggle the comment character at the start of the line.
# This is meant to work around a buggy implementation of pound-insert in zsh.
#
...
for keymap in 'emacs' 'viins' 'vicmd'; do
  ...
  bindkey -M "$keymap" '^H' history-incremental-search-backward
  bindkey -M "$keymap" '^S' history-incremental-search-forward
  bindkey -M "$keymap" '^P' history-search-backward
  bindkey -M "$keymap" '^N' history-search-forward
done
...
for keymap in 'emacs' 'viins'; do
  ...
  for key in "$key_info[Escape]"{W,w}
    bindkey -M "$keymap" "$key" kill-region
