...
package.path = package.path .. ";" .. DOTFILES_DIR ..
                   "/darwin/modules/hammerspoon/?.lua"
package.path = package.path .. ";" .. DOTFILES_DIR ..
                   "/darwin/modules/hammerspoon/?/init.lua"
package.cpath = package.cpath .. ";" .. DOTFILES_DIR ..
                    "/darwin/modules/hammerspoon/?.so"
...
configwatcher = pathwatcher.new(hs.configdir .. "/init.lua", function()
    if configwatcher then hs.reload() end
end)
configwatcher:start()
...
local launchOrFocus = function(name) return application.launchOrFocus(name) end -- wrap to discard extra args (i.e. fnutils.some passes index)
local launchOrFocusByBundleID = function(id)
    return application.launchOrFocusByBundleID(id)
end
local appNameMatcher = function(name)
    return function(app) return app and app:name():find(name) end
end
local appBundleIDMatcher = function(bundleID)
    return function(app) return app and app:bundleID():find(bundleID) end
end
local appSwitcher = function(opts)
    local bundleIDs = type(opts.bundleID) == "string" and {opts.bundleID} or
                          opts.bundleID or {}
    local names = type(opts.name) == "string" and {opts.name} or opts.name or {}

    local matchers = fnutils.concat(fnutils.map(bundleIDs, appBundleIDMatcher),
                                    fnutils.map(names, appNameMatcher))

    if #matchers == 0 then
        error(
            "appSwitcher requires at least one of 'name' or 'bundleID' in opts")
    end
    ...
    local currentlyFocused = function()
        ...
        if win then
            ...
            if fnutils.some(matchers, function(f) return f(app) end) then
                return true
            end
        end
        return false
    ...
    local launch = function()
        return (fnutils.some(bundleIDs, launchOrFocusByBundleID) or
                   fnutils.some(names, launchOrFocus))
    end
    ...
    return function()
        ...
        if opts.callback then opts.callback(opts) end
    end
end
...
local aerospaceReloadConfig = function()
    ...
    if ok then
        ...
        if ok then alert("✅ Aerospace") end
    end
end
...
modes.main:bind(HYPER, "a", appSwitcher(
                    {bundleID = "com.fastmail.mac.Fastmail", name = "Fastmail"}))
...
modes.main:bind(HYPER, "m", appSwitcher(
...
modes.main:bind(HYPER, "f5", hs.reload)
modes.main:bind(HYPER, "f6", aerospaceReloadConfig)
modes.main:bind(HYPER, "f7",
                partial(hs.execute, "aerospace enable toggle", true))
